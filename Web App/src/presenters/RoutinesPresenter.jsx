...
import {
  ...
  setMustBeInBedTime,
  setRoutineSettings,
} from "../model/modules/routines";

function RoutinePresenter() {
  const dispatch = useDispatch();

  const reduxMaxTimeInBed = useSelector(
    (state) => state.routine.max_time_in_bed
  );
  const reduxMustBeInBedTime = useSelector(
    (state) => state.routine.must_be_in_bed_time
  );
  ...
  return (
    <PageView title="Routine Settings">
      <RoutinesView
