		...
		<div className="h-full">
			<div className="flex flex-col gap-4">
				<div className="bg-white rounded-lg shadow-sm border border-gray-200">
					...
					<div className="px-4 pb-4">
						<div className="flex flex-row ">
							...
							<Slider value={[props.brightness]} max={100} step={1} onValueChange={(value) => changeBrightnessACB(value[0])} />
					...
					<div>
						<div className="flex flex-row p-4 gap-4 justify-center">
							{props.colors.map((color) => (
								<div key={color.id} className="w-15 h-15 rounded-lg" style={{ backgroundColor: color.color_hex }}>
				...
				<div className="bg-white rounded-lg shadow-sm border border-gray-200">
					<div className="p-4 flex items-center gap-3">
						<Sunrise size={20} />
						<h2 className="text-lg font-semibold text-gray-800">Sunrise settings</h2>
					</div>
					<div className="px-4 pb-4">
						<div className="flex flex-row ">
							<div className="pr-4">0</div>
							<Slider value={[props.sunrise]} max={60} step={5} onValueChange={(value) => changeSunriseACB(value[0])} />
							<div className="pl-4">60</div>
						</div>
					</div>
					<div className="text-xs p-4 flex flex-row">
						<div className="pr-1 font-bold">{props.sunrise} </div>
						<div>minutes before {props.brightness?.brightness || props.brightness}% brightness</div>
					</div>
				</div>
			</div>
		</div>
